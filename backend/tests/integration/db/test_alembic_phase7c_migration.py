...
def test_upgrade_idempotent_no_duplicate_seed(tmp_path):
    ...
    with eng.connect() as conn:
        count = conn.execute(
            text("SELECT COUNT(*) FROM api_keys WHERE name='bootstrap-admin'")
        ).scalar()
    assert count == 1
    eng.dispose()
...
def test_existing_jobs_survive_downgrade(tmp_path):
    ...
    with eng.begin() as conn:
        conn.execute(
            text(
                ...
                " (id, name, model, backend, connection, enabled, created_at, updated_at)"
        ...
        conn.execute(
            text(
                "INSERT INTO jobs"
                " (id, printer_id, template_key, state, payload, created_at, updated_at)"
    ...
    with eng2.connect() as conn:
        count = conn.execute(
            text("SELECT COUNT(*) FROM jobs WHERE id='bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'")
        ).scalar()
    assert count == 1
    eng2.dispose()
