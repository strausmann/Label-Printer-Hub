...
async def _create_printer(session: AsyncSession) -> Printer:
    ...
    p = Printer(name=f"Test-Printer-{uuid4().hex[:8]}", slug=f"test-{uuid4().hex[:8]}",
...
async def test_create_and_get(db_session: AsyncSession):
    ...
    b = PrintBatch(id=batch_id, printer_id=printer.id, job_ids=job_ids,
                   created_by="björn@example.test")
...
async def test_list_recent_24h(db_session: AsyncSession):
    ...
    b1 = PrintBatch(id=uuid4(), printer_id=printer1.id, job_ids=["j1"],
                    created_by="u", created_at=now - timedelta(hours=2))
    b2 = PrintBatch(id=uuid4(), printer_id=printer2.id, job_ids=["j2"],
                    created_by="u", created_at=now - timedelta(hours=48))
