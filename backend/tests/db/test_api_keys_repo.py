...
def _make_key(*, name="test-key", key_hash="\$2b\$12\$fake", key_prefix="lh_ab12cd34",
              scopes=None, allowed_printer_ids=None, rate_limit_per_minute=60,
              enabled=True, expires_at=None) -> ApiKey:
    return ApiKey(
        name=name, key_hash=key_hash, key_prefix=key_prefix,
        scopes=scopes or ["read"], allowed_printer_ids=allowed_printer_ids or [],
        rate_limit_per_minute=rate_limit_per_minute, enabled=enabled, expires_at=expires_at,
    )
...
async def test_list_active_returns_only_enabled_non_expired(session):
    ...
    expired = _make_key(name="expired", key_prefix="lh_cccccccccc", enabled=True,
                        expires_at=datetime.now(UTC) - timedelta(hours=1))
    future = _make_key(name="future-expiry", key_prefix="lh_dddddddddd", enabled=True,
...
async def test_update_last_used_sets_timestamp_and_ip(session):
    ...
    luat = updated.last_used_at.replace(tzinfo=None) if updated.last_used_at.tzinfo else updated.last_used_at
