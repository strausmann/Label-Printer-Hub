...
def _sample_item() -> PrintRequest:
    return PrintRequest(
        ...
        data=RawLabelData(title="Kallax 10 Fach 2-3",
...
def test_batch_response_with_all_succeeded():
    resp = BatchResponse(
        batch_id=uuid4(), printer_id=uuid4(), queued_at="2026-05-30T19:42:01Z",
        job_ids=[str(uuid4()) for _ in range(5)], errors=[])
    assert len(resp.job_ids) == 5
    assert resp.errors == []


def test_batch_error_with_required_fields():
    err = BatchError(index=3, error_code="template_not_found",
                     error_message="Template 'x' does not exist")
    assert err.index == 3
